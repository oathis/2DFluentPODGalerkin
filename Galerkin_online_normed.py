...
def get_alpha_stats(offline_data, K):
    """오프라인 단계에서 저장한 평균/표준편차 정보를 불러옵니다."""
    alpha_mean = offline_data.get('alpha_mean')
    alpha_std = offline_data.get('alpha_std')

    if alpha_mean is None or alpha_std is None:
        print("[Info] alpha_mean/alpha_std not found in offline data. Using zeros/ones as fallback.")
        alpha_mean = np.zeros(K)
        alpha_std = np.ones(K)

    alpha_mean = np.asarray(alpha_mean, dtype=float).reshape(K)
    alpha_std = np.asarray(alpha_std, dtype=float).reshape(K)
    alpha_std_safe = np.where(alpha_std > 1e-12, alpha_std, 1.0)
    return alpha_mean, alpha_std, alpha_std_safe


def evaluate_rom_residual(alpha_scaled, Re, C1, C2, L1, L2, Q, alpha_mean, alpha_scale, residual_scale):
    """스케일링된 변수(alpha_scaled)를 실제 계수(alpha)로 변환 후 잔차를 계산합니다."""
    alpha = alpha_mean + alpha_scale * alpha_scaled
    C = C1 + (1 / Re) * C2
    L = L1 + (1 / Re) * L2
    linear_term = L @ alpha
    quadratic_term = np.einsum('mij,i,j->m', Q, alpha, alpha)
    residual_raw = C + linear_term + quadratic_term
    residual_scaled = residual_raw / residual_scale
    return residual_scaled, residual_raw, alpha


def rom_residual(alpha_scaled, Re, C1, C2, L1, L2, Q, alpha_mean, alpha_scale, residual_scale):
    """fsolve에 전달되는 잔차 함수 (스케일링 적용)."""
    residual_scaled, _, _ = evaluate_rom_residual(
        alpha_scaled, Re, C1, C2, L1, L2, Q, alpha_mean, alpha_scale, residual_scale
    )
    return residual_scaled



def residual_with_monitor(alpha_scaled, Re, C1, C2, L1, L2, Q, alpha_mean, alpha_scale, residual_scale):
    """
    ...
    if not hasattr(residual_with_monitor, "iteration"):
        ...
        print(" Iter | Raw Residual | Scaled Residual")
        print("---------------------------------------")

    residual_scaled, residual_raw, _ = evaluate_rom_residual(
        alpha_scaled, Re, C1, C2, L1, L2, Q, alpha_mean, alpha_scale, residual_scale
    )
    norm_raw = np.linalg.norm(residual_raw)
    norm_scaled = np.linalg.norm(residual_scaled)
    print(f" {residual_with_monitor.iteration:4d} | {norm_raw:.6e} | {norm_scaled:.6e}")
...
def run_online_stage(re_input, output_dir=None):
    ...
    try:
        ...
        alpha_mean, alpha_std_raw, alpha_std_safe = get_alpha_stats(offline_data, K)
        if np.any(alpha_std_raw < 1e-12):
            print("[Info] Some mode standard deviations are near zero; unit scaling applied for those modes.")
        residual_scale = alpha_std_safe

        alpha_initial_guess = np.zeros(K)  # normalized space (alpha_scaled)
        args = (
            Re,
            offline_data['C1'], offline_data['C2'],
            offline_data['L1'], offline_data['L2'],
            offline_data['Q'],
            alpha_mean, alpha_std_safe, residual_scale
        )
        
        if hasattr(residual_with_monitor, "iteration"):
            del residual_with_monitor.iteration

        alpha_solution_scaled, info, ier, msg = fsolve(
            residual_with_monitor,
            alpha_initial_guess,
            args=args,
            full_output=True,
            xtol=1e-12
        ...
        alpha_solution = alpha_mean + alpha_std_safe * alpha_solution_scaled

        print(f"\nSolved alpha coefficients (scaled back): {alpha_solution}")
