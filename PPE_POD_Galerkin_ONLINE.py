# online.py — Steady PPE-ROM (Lifting + POD + Galerkin, with constant/linear/quadratic split)
# @Kim Jae Sik
...
# === DEBUG: Residual Block Decomposition ===
def residual_blocks_with_parts(z, Re, offline, use_boundary_term=True, scaled=True):
    ...
    # Return L2 norms of each scaled term
...
# === DEBUG: Jacobian Condition Number/Minimum Singular Value ===
...
def jacobian_split(z, Re, offline, use_boundary_term=True):
    ...
    Block structure:
      [ dRu/da  dRu/db ]
      [ dRp/da  dRp/db ]
    Note: Must apply the same scaling (Ru_scale, Rp_scale) as in the residual.
    """
    ...
    # dRu/da = nu*A + L + d(Q(a,a))/da
    term1 = np.tensordot(Q, a, axes=(2, 0))      # term1[m,l] = sum_k Q[m,l,k] a_k
    term2 = np.tensordot(Q, a, axes=(1, 0))      # term2[m,l] = sum_k Q[m,k,l] a_k
    dRu_da = + nu * A + L + (term1 + term2)

    # dRu/db = B
    dRu_db = B

    # dRp/da = G1 + d(G2(a,a))/da - nu*N1
    H1 = np.tensordot(G2, a, axes=(2, 0))        # H1[i,l] = sum_k G2[i,l,k] a_k
    H2 = np.tensordot(G2, a, axes=(1, 0))        # H2[i,l] = sum_k G2[i,k,l] a_k
    dRp_da = G1 + (H1 + H2)
...
def solve_steady_rom(Re, offline, z0=None, tol=1e-10, max_iter=10000, use_boundary_term=True):
    ...
    def wrap(z):
        ...
        # --- summary output (every 5 iterations) + Jacobian state ---
        if wrap.it % 5 == 0:
    ...
    if ier != 1:
        ...
        # Print final decomposition on failure
