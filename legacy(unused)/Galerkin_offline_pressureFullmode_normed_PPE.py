...
def extract_velocity_interior(field_1d, nx, ny):
    """경계(Dirichlet) 영역을 제외한 내부 속도 데이터만 추출합니다."""
    field_2d = field_1d.reshape((ny, nx))
    return field_2d[1:-1, 1:-1].flatten()


def extract_pressure_full(field_1d):
    """압력은 푸아송 방정식 경계 조건 때문에 전체 격자를 유지합니다."""
    return field_1d.copy()
...
def visualize_interior_snapshot(Q_int_col, nx, ny, case_index, data_directory):
    ...
    # 1차원 벡터에서 p, u, v 분리
    p_full = Q_int_col[0:n_nodes]
    u_int = Q_int_col[n_nodes:n_nodes + n_nodes_int]
    v_int = Q_int_col[n_nodes + n_nodes_int: n_nodes + 2 * n_nodes_int]
...
def run_offline_stage(data_directory=None):
    ...
    nx_int, ny_int = NX - 2, NY - 2
    n_nodes_int = nx_int * ny_int
    Q_interior = np.zeros((N_NODES + 2 * n_nodes_int, actual_num_cases))
    for i in range(actual_num_cases):
        p, u, v = Q[0:N_NODES, i], Q[N_NODES:2*N_NODES, i], Q[2*N_NODES:3*N_NODES, i]
        # 압력: 경계 조건과의 결합을 위해 전체 노드를 유지합니다.
        p_full = extract_pressure_full(p)
        # 속도: 주변 경계는 Dirichlet 값으로 고정되므로 내부 셀만 POD에 사용합니다.
        u_int = extract_velocity_interior(u, NX, NY)
        v_int = extract_velocity_interior(v, NX, NY)
        Q_interior[:, i] = np.concatenate([p_full, u_int, v_int])
    ...
    # 각 모드를 p, u, v 부분으로 분리
    p_modes_full = Phi_interior[0:N_NODES, :]
    u_modes_int = Phi_interior[N_NODES:N_NODES + n_nodes_int, :]
    v_modes_int = Phi_interior[N_NODES + n_nodes_int:N_NODES + 2 * n_nodes_int, :]
    ...
    for k in range(K):
        ...
        p_energy = np.linalg.norm(p_modes_full[:, k])**2
    ...
    u_modes_int = Phi_interior[N_NODES:N_NODES + n_nodes_int, :]
    v_modes_int = Phi_interior[N_NODES + n_nodes_int:N_NODES + 2 * n_nodes_int, :]
    p_modes = p_modes_full
    u_modes = expand_modes(u_modes_int, NX, NY)
    v_modes = expand_modes(v_modes_int, NX, NY)

    # 5. 경계 조건(u_bc) 생성
    print("5. Creating boundary condition vector u_bc...")
    u_bc = np.zeros(N_NODES)
    y_coords = coords[:, 1].reshape((NY, NX))
    u_bc[(y_coords == y_coords.max()).flatten()] = 1
    ...
    for i in range(K):
        ...
        lap['p'].append(deriv.laplacian(p_modes[:, i]))
    ...
    for m in range(K):
        # Pressure Poisson equation: \nabla^2 p + (\partial_x u)^2 + 2 (\partial_y u)(\partial_x v) + (\partial_y v)^2 = 0
        # Constant contribution comes solely from the boundary-driven streamwise gradient.
        poisson_const = u_bc_dx ** 2
        C1[m] = np.dot(poisson_const, p_modes[:, m]) + np.dot(u_bc * u_bc_dx, u_modes[:, m])
        C2[m] = np.dot(-u_bc_lap, u_modes[:, m])
        
        for j in range(K):
            poisson_linear = lap['p'][j] + 2 * u_bc_dx * d_dx['u'][j] + 2 * u_bc_dy * d_dx['v'][j]
            L1_rc = np.dot(poisson_linear, p_modes[:, m])
            ...
            for i in range(K):
                ...
                Q_rp = np.dot(poisson_quad_term, p_modes[:, m])

                Q[m, i, j] = Q_ru + Q_rv + Q_rp
