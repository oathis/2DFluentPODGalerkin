...
def run_offline_stage(data_directory=None):
    ...
    for i in range(actual_num_cases):
        ...
        p_full = p.reshape(NY, NX).flatten()
    ...
    v_bc_dx = np.zeros_like(u_bc_dx)
    v_bc_dy = np.zeros_like(u_bc_dy)
    ...
    poisson_const = u_bc_dx**2 + v_bc_dy**2 + 2 * u_bc_dy * v_bc_dx
    for m in range(K):
        ...
        for j in range(K):
            ...
            for i in range(K):
                ...
                Q_rp = np.dot(poisson_quad, p_modes[:, m])
