...
def solve_steady_rom(Re, offline, z0=None, tol=1e-10, max_iter=10000, use_boundary_term=True):
    ...
    def wrap(z):
        Ru, Rp, nRu, nRp = residual_blocks_with_parts(z, Re, offline, use_boundary_term, scaled=True)
        r = np.concatenate([Ru, Rp])
        if not hasattr(wrap, "it"):
            wrap.it = 0
            print("\n--- fsolve residual (scaled) ---")
            print(" iter |  ||Ru||   ||Rp||   ||r||   |  Ru terms [nuA+fA, c0, c_p, La, Qa, Bb]  |  Rp terms [Db, g0, G1a, G2aa, -nuN, F]")
            print("-------------------------------------------------------------------------------------------------------------")
        if wrap.it % 5 == 0:  # 5회마다 요약
