# offline.py — Steady PPE-ROM (Lifting + POD + Galerkin, no p_bar, no gauge)
...
# --- Operator assembly (no p_bar/F/c_p) ---
def assemble_operators_lifting(phi_u, phi_v, chi, u_bc, v_bc, deriv:DerivativeHelper):
    ...
    # Convective split: c0, L, Q  (divergence form)
    ...
    # PPE source split: g0, G1, G2  (no F)
    ...
    return dict(A=A, fA=fA, B=B, Q=Q, L=L, c0=c0,
                D=D, G2=G2, G1=G1, g0=g0, N1=N1, N0=N0)

def main(data_directory: str = DEFAULT_DATA_DIRECTORY):
    t0 = time.time()
    print("OFFLINE: Steady PPE-ROM (no p_bar, pressure = pure SVD)")
    ...
    # Velocity fluctuation snapshots (lifting)
    Qu_fluc = Qu.copy()
    Qu_fluc[:N_NODES,:]   -= u_bc[:,None]
    Qu_fluc[N_NODES:,:]   -= v_bc[:,None]

    # --- Pressure: pure SVD (no p_bar, no gauge) ---
    print("POD (SVD) ...")
    Up, sp, _ = svd(Qp, full_matrices=False)          # <== 그대로 SVD
    Uu, su, _ = svd(Qu_fluc, full_matrices=False)
    Kp = min(K_PRS, Up.shape[1]); Ku = min(K_VEL, Uu.shape[1])

    chi = Up[:, :Kp]                                   # pressure basis
    phi_uv = Uu[:, :Ku]                                # velocity basis
    ...
    ops = assemble_operators_lifting(phi_u, phi_v, chi, u_bc, v_bc, deriv)

    # Block scales (optional)
